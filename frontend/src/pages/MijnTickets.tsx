...
const MijnTickets: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getErrorMessage(e, 'Ticket indienen mislukt'));
    } finally {
      setSubmitting(false);
    }
  };
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-xs ${
                        t.urgency === 'DRINGEND' ? 'bg-red-100 text-red-800' :
                        t.urgency === 'NORMAAL' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                        {t.urgency}
                      </span>
